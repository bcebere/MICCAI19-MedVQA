...
def get_result(model, dataloader, device, args):
    ...
    question_types_result = dict(
        (i, dict((j, dict((k, 0.0) for k in keys)) for j in quesntion_types))
        ...
        for i in answer_types:
            ...
            for j in quesntion_types:
                if question_types_result[i][j]["count"] != 0.0:
                    question_types_result[i][j]["score"] = (
                        question_types_result[i][j]["true"]
                        / question_types_result[i][j]["count"]
                    )
                    question_types_result[i][j]["score_percent"] = round(
                        question_types_result[i][j]["score"] * 100, 1
                    )
                if question_types_result[i][j]["real"] != 0.0:
                    question_types_result[i][j]["real_percent"] = round(
                        question_types_result[i][j]["real"]
                        / question_types_result[i][j]["count"]
                        * 100.0,
                        1,
                    )
...
if __name__ == "__main__":
    ...
    def save_questiontype_results(outfile_path, quesntion_types_result):
        for i in quesntion_types_result:
            pd.DataFrame(quesntion_types_result[i]).transpose().to_csv(
                outfile_path + "/question_type_" + i + ".csv"
            )
    ...
    def process(args, model, eval_loader):
        ...
        if args.use_RAD:
            result, quesntion_types_result = get_result(
                model, eval_loader, args.device, args
            )
